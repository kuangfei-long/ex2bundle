...
Same report as experiments/item6_bound_direction/full_report.py, but driven
by ex2bundle_original.Ex2Bundle -- the restored, unmodified pre-refactor
model that supports relaxation_mode/bound_pad and the Item-6 instrumentation
(last_objective, last_relax_magnitude, ...) natively. No hooks, no
subclassing, no monkey-patching: this folder exists so the three relaxation
strategies can be exercised directly, exactly as originally implemented.
...
Usage as a library
-------------------
    from experiments.item6_bound_direction_original.full_report import (
...
Usage from the CLI
------------------
    python experiments/item6_bound_direction_original/full_report.py \\
        --data_path data/data_ctm.csv --shared_docs data/shared_docs/ \\
...
from experiments.item6_bound_direction_original.ex2bundle_original import Ex2Bundle, VALID_RELAXATION_MODES

MODES = VALID_RELAXATION_MODES  # ("symmetric", "directional", "feasopt")

_QUALITY_KEYS = ("relax_freq", "rouge1_f1", "rouge2_f1", "rougeL_f1", "sbert", "bound_shift", "num_relaxed", "objective")
...
def _pairwise_vs_baseline(raw_rows, pad, modes, baseline="symmetric", tol=1e-6):
    """Pairwise directional-vs-symmetric / feasopt-vs-symmetric comparison for
    this bound_pad, restricted to queries that needed relaxation under BOTH
    sides. Objective: higher wins. Runtime: lower wins. Keyed by
    (intent_idx, target) so a dropped instance in one mode doesn't misalign
    the pairing."""
...
def run_full_item6_report(data_path, shared_docs, users_path,
    ...
    dict:
      "quality": {bound_pad: {mode: {n, relax_rate, rouge1_f1, rouge2_f1,
        ...
        -- objective_*/runtime_* are directional-vs-symmetric and
        feasopt-vs-symmetric pairwise comparisons (symmetric maps to "-"/nan),
        restricted to queries that needed relaxation under both sides.
      ...
      "infeasible": {bound_pad: {mode: {n, rouge1_f1, rouge2_f1, rougeL_f1,
        ...
        -- same quality metrics, averaged only over relaxed instances.
      "raw": [ {bound_pad, mode, intent_idx, target, rouge1_f1, rouge2_f1,
        ...
        -- one row per (bound_pad, mode, instance), pre-aggregation.
      "n_instances", "bound_pads", "modes"
    """
    for m in modes:
        assert m in VALID_RELAXATION_MODES, f"unknown mode {m}"
    ...
    for pad in bound_pads:
        models = {m: Ex2Bundle(data_path, shared_docs, n_topics, num_examples,
                                is_generative=False, relaxation_mode=m, bound_pad=pad)
                  for m in modes}
        ...
        for inst in instances:
            ...
            for m in modes:
                ...
                bound_shift = models[m].last_relax_magnitude   # native instrumentation
                objective = models[m].last_objective            # native instrumentation
        ...
        for m in modes:
            ...
            quality[pad][m] = {
                ...
                **_mean_stats(recs),  # includes relax_freq, meaned over ALL recs
                **pairwise[m],
            }
...
def write_summary_csv(report, path):
    """One row per (bound_pad, mode): #constraints relaxed, total
    bound-relaxation amount, objective score, and ROUGE/SBERT -- averaged
    only over instances that actually required relaxation (relax_freq > 0),
    same subset the clean-timing columns are measured on. relax_rate is the
    one exception: it's a rate OVER ALL instances (fraction that needed
    relaxation), so it's the only field still sourced from the full cell."""
    os.makedirs(os.path.dirname(path) or ".", exist_ok=True)
    with open(path, "w", newline="") as f:
        ...
        for pad in report["bound_pads"]:
            for m in report["modes"]:
                ...
                inf = report["infeasible"][pad][m]
                rt = report["runtime"][pad][m]
                w.writerow({
                    "bound_pad": pad, "mode": m, "n": inf["n"],
                    "relax_rate": q["relax_rate"], "relax_freq": inf["relax_freq"],
                    "rouge1_f1": inf["rouge1_f1"], "rouge2_f1": inf["rouge2_f1"],
                    "rougeL_f1": inf["rougeL_f1"], "sbert": inf["sbert"],
                    "bound_shift": inf["bound_shift"], "num_relaxed": inf["num_relaxed"],
                    "objective": inf["objective"],
...
def main():
    ...
    ap.add_argument("--output_file", default="results/Item6/full_report_original/full_report",
